 ...
 * @example When starting a ray at a surface, it can happen that the surface
 * reports a self-intersection. Mathematically, the intersection distance should
 * be zero, allowing us to discard the intersection, but in reality floating
 * point inaccuracies can report larger distances that would be considered valid
 * intersections. This threshold allows us to solve this (and many other)
 * inaccuracy problem, by treating anything below this threshold as zero.
 */
...
public:
    ...
    /// @brief Constructs a point from one of dimensionality one less, and a
    ...
    /// @brief Constructs a four-dimensional point (used for homogeneous
    /// coordinates).
    TPoint(const Type &x, const Type &y, const Type &z, const Type &w)
        : m_data({ x, y, z, w }) {}
    ...
    template <typename Type2> auto cast() const {
        TPoint<Type2, Dimension> BUILD1(Type2(m_data[i]))
    }
...
public:
    ...
    friend auto elementwiseMin(const TVector &a, const TVector &b) {
        TVector BUILD1(std::min(a[i], b[i]))
    }
    /// @brief Returns the elementwise maximum of two vectors.
    friend auto elementwiseMax(const TVector &a, const TVector &b) {
        TVector BUILD1(std::max(a[i], b[i]))
    }
    ...
    auto minComponentIndex() const {
        return (int) std::distance(
            m_data.begin(), std::min_element(m_data.begin(), m_data.end()));
    }
    /// @brief Returns the index of the highest component of the vector.
    auto maxComponentIndex() const {
        return (int) std::distance(
...
public:
    ...
    TMatrix(std::initializer_list<Type> l) {
        assert(l.size() == (Rows * Columns)); // "cannot initialize Matrix<%d,
                                              // %d> with %d elements", Rows,
                                              // Columns, l.size()
    ...
    /// @brief Returns a row of the matrix as vector, with index ranging from @c
    /// 0 to @code Rows - 1 @endcode .
    auto row(int rowIndex) const {
        TVector<Type, Columns> BUILD1((*this)(rowIndex, i))
    }
    /// @brief Returns a column of the matrix as vector, with index ranging from
    /// @c 0 to @code Columns - 1 @endcode .
    auto column(int columnIndex) const {
        TVector<Type, Rows> BUILD1((*this)(i, columnIndex))
    }

    /// @brief Sets a row of the matrix to a given vector, with index ranging
    /// from @c 0 to @code Rows - 1 @endcode .
    auto setRow(int rowIndex, const TVector<Type, Columns> &vector) {
        for (int c = 0; c < Columns; c++)
            (*this)(rowIndex, c) = vector[c];
    }
    /// @brief Sets a column of the matrix to a given vector, with index ranging
    /// from @c 0 to @code Columns - 1 @endcode .
    ...
    auto operator*(const TMatrix<Type, Columns, Columns2> &other) const {
        TMatrix<Type, Rows, Columns2> BUILD2(
            (*this).row(row).dot(other.column(column)))
    }

    /// @brief Checks whether two matrices are exactly identical.
    bool operator==(const TMatrix &other) const {
        return m_data == other.m_data;
    }
    /// @brief Checks whether two matrices are not exactly identical.
    bool operator!=(const TMatrix &other) const {
        return m_data != other.m_data;
    }
    ...
    /// @brief Returns a slice of this matrix of size @c Rows2 x @c Columns2 ,
    /// offset by @c startRow and @c startColumn .
    ...
    auto submatrix(int startRow, int startColumn) const {
        ...
        TMatrix<Type, Rows2, Columns2> BUILD2(
            (*this)(startRow + row, startColumn + column))
    }
    ...
    friend auto operator*(const Type &a, const TMatrix &b) {
        TMatrix BUILD2(a * b(row, column))
    }
    friend auto operator*(const TMatrix &a, Type b) {
        TMatrix BUILD2(a(row, column) * b)
    }
    friend auto operator+(const TMatrix &a, const TMatrix &b) {
        TMatrix BUILD2(a(row, column) + b(row, column))
    }
    friend auto operator-(const TMatrix &a, const TMatrix &b) {
        TMatrix BUILD2(a(row, column) - b(row, column))
    }
...
/**
 * @brief Bounds describe the range from one point to another (e.g., an
 * axis-aligned bounding box). In one dimension, this corresponds to an interval
...
public:
    ...
    TBounds(const Point &min, const Point &max) : m_min(min), m_max(max) {}
    ...
    /**
     * @brief Returns whether the bounding box covers an infinite amount of
     * space.
     * @note It suffices if a single axis of the bounding box covers an infinite
     * range.
     */
    bool isUnbounded() const {
        for (int dim = 0; dim < Dimension; dim++) {
            if (m_min[dim] < 0 && std::isinf(m_min[dim]))
                return true;
            if (m_max[dim] > 0 && std::isinf(m_max[dim]))
                return true;
        }
        return false;
    }

    /// @brief Clamps the components of a point so that it lies within this
    /// bounding box.
    Point clip(const Point &point) const {
        return elementwiseMax(m_min, elementwiseMin(point, m_max));
    }

    /// @brief Clamps the components of another bounding box so that it lies
    ...
    /// @brief Tests for equality with another bounds object.
    bool operator==(const TBounds &other) const {
        return m_min == other.m_min && m_max == other.m_max;
    }

    /// @brief Tests for inequality with another bounds object.
    bool operator!=(const TBounds &other) const {
        return m_min != other.m_min || m_max != other.m_max;
    }
    ...
    bool includes(const Point &point) const {
        for (int dim = 0; dim < D; dim++) {
            if (point[dim] < m_min[dim])
                return false;
            if (point[dim] > m_max[dim])
                return false;
        }
        return true;
    }
...
/// @brief A four-dimensional vector with floating point components (used for
...
/// @brief A 4x4 matrix with floating point components (used for homogeneous
/// coordinates).
using Matrix4x4 = TMatrix<float, 4, 4>;

/// @brief Computes the component-wise modulo operation of two points.
template <int Dimension>
auto operator%(const TPoint<int, Dimension> &a,
               const TPoint<int, Dimension> &b) {
    TPoint<int, Dimension> BUILD1(a[i] % b[i])
}
...
void buildOrthonormalBasis(const Vector &normal, Vector &tangent,
                           Vector &bitangent);
...
/**
 * @brief Refracts a vector @c w at a surface with normal @c n and relative
 * index of refraction @c eta .
 * @note All vectors are assumed to point away from the surface.
 * @return Refracted vector, or zero vector in case of total internal
 * reflection.
 */
inline Vector refract(const Vector &w, const Vector &n, float eta) {
    ...
    const float cosTheta = n.dot(w);
    return (invEta * cosTheta - copysign(sqrt(k), cosTheta)) * n - invEta * w;
}
...
struct Ray {
    ...
    Vector direction;
    /// @brief The number of bounces encountered by the ray, for use in
    /// integrators.
    int depth = 0;
    ...
    /// @brief Returns a copy of the ray with normalized direction vector
    /// (useful after applying transforms).
    Ray normalized() const {
        return Ray(origin, direction.normalized(), depth);
    }
};

/**
 * @brief Defines shading frames and common trigonometrical functions used
 * within them. In lightwave, we follow the convention that material functions
 * (sampling and evaluation of Bsdfs and Emissions) happen within a local frame
 * where the normal vector is always [0,0,1], which simplifies much of their
 * implementation. This class defines the orientation of a surface, which allows
 * us to transform world space vectors (e.g., ray directions) into this local
 * shading frame.
 * @warning Make sure that the vectors that constitute your Frame are always
 * orthonormal and form a right-handed coordinate system.
 */
struct Frame {
    /// @brief The normal vector of the surface (implicitly given by the cross
    /// product of tangent and bitangent).
    Vector normal;
    /// @brief The tangent vector, which lies in the surface.
    Vector tangent;
    /// @brief The bitangent vector, which lies in the surface and is orthogonal
    /// to the tangent vector.
    Vector bitangent;

    Frame() {}

    /// @brief Constructs a frame with arbitrary tangent and bitangent vector
    /// from a given normal vector.
    explicit Frame(const Vector &normal) : normal(normal) {
        buildOrthonormalBasis(normal, tangent, bitangent);
    }

    Frame(const Vector &tangent, const Vector &bitangent, const Vector &normal)
        : normal(normal), tangent(tangent), bitangent(bitangent) {}

    /// @brief Converts a vector from world-space coordinates to local shading
    /// coordinates.
    ...
    /// the angle they form is below 180°).
    ...
    static float sinTheta(const Vector &w) {
        return safe_sqrt(1 - cosTheta2(w));
    }
    ...
    /// @brief Asserts that the frame is orthonormal and aborts if that is not
    /// the case.
    void validate() const;
};
...
/// [0,1] returns c).
template <typename T>
static T interpolateBarycentric(const Vector2 &bary, const T &a, const T &b,
                                const T &c) {
...
/// [0,1] returns c).
template <>
Point interpolateBarycentric(const Vector2 &bary, const Point &a,
...
struct Vertex {
    ...
    Vector2 uv;
    /// @brief The normal vector, which will be barycentrically interpolated
    /// when smooth normals are used.
    Vector normal;

    /// @brief Barycentric interpolation of vertices ([0,0] returns a, [1,0]
    /// returns b, and [0,1] returns c).
    static Vertex interpolate(const Vector2 &bary, const Vertex &a,
...
/// @brief A point on a surface along with context about the orientation of the
/// surface.
struct SurfaceEvent {
    ...
    /// @brief The probability of sampling the point when doing area sampling,
    /// in area units.
    float pdf;
...
struct Intersection : public SurfaceEvent {
    /// @brief The direction of the ray that hit the surface, pointing away from
    /// the surface.
    Vector wo;
    /// @brief The intersection distance, which can also be used to specify a
    /// maximum distance when querying intersections.
    float t;
    /**
     * @brief The probability of having picked the intersected light source
     * using
     * @c Scene::sampleLight , or zero if no light source was intersected.
     */
    float lightProbability;
    /**
     * @brief The background of the scene, only set in case no object was hit
     * and the scene has defined one.
     */
    BackgroundLight *background = nullptr;

    /// @brief Statistics recorded while traversing acceleration structures.
    struct {
        /// @brief The number of BVH nodes that have been tested for
        /// intersection.
        int bvhCounter = 0;
    ...
    EmissionEval evaluateEmission() const;
...
static std::ostream &operator<<(std::ostream &os,
                                const TPoint<Type, Dimension> &point) {
    os << "Point[";
    for (int i = 0; i < Dimension; i++) {
        if (i)
            os << ", ";
...
template <typename Type, int Dimension>
static std::ostream &operator<<(std::ostream &os,
                                const TVector<Type, Dimension> &vector) {
    os << "Vector[";
    for (int i = 0; i < Dimension; i++) {
        if (i)
            os << ", ";
...
static std::ostream &operator<<(std::ostream &os,
                                const TMatrix<Type, Rows, Columns> &matrix) {
    os << "Matrix[" << std::endl;
    for (int row = 0; row < Rows; row++) {
        os << "  ";
        for (int column = 0; column < Columns; column++) {
...
/// @brief Print a given ray to an output stream.
...
/// @brief Print a given frame to an output stream.
static std::ostream &operator<<(std::ostream &os, const Frame &frame) {
    os << "Frame[t=" << frame.tangent << ", b=" << frame.bitangent
       << ", n=" << frame.normal
       << "; |t*b|=" << frame.tangent.dot(frame.bitangent)
       << ", |t*n|=" << frame.tangent.dot(frame.normal)
       << ", |b*n|=" << frame.bitangent.dot(frame.normal) << "]";
    return os;
}
...
template <typename F, int D> bool isfinite(const lightwave::TPoint<F, D> &p) {
    for (int dim = 0; dim < D; dim++)
...
/// @brief Tests whether all components of a given vector are finite.
template <typename F, int D> bool isfinite(const lightwave::TVector<F, D> &p) {
    for (int dim = 0; dim < D; dim++)
        if (!std::isfinite(p[dim]))
            return false;
    return true;
...
template <typename T> inline bool check_finite(const T &n) {
    return std::isfinite(n);
}

template <typename T> inline bool check_normalized(const T &n) {
    const float lenSqr = n.lengthSquared();
    return abs(lenSqr - 1) < 1e-3f;
}

template <typename T> inline bool check_orthogonal(const T &a, const T &b) {
    const float dotProduct = a.dot(b);
    return abs(dotProduct) < 1e-3f;
}

// #ifdef LW_DEBUG
//  lightwave with assertions enabled is not much slower, so we also keep them
...
/// note the case.
#define assert_condition(condition, ctx)                                       \
    if (!(condition)) {                                                        \
        logger(EError,                                                         \
               "%s:%d : " #condition " is false",                              \
               relpath(__FILE__),                                              \
               __LINE__);                                                      \
        ctx;                                                                   \
        abort();                                                               \
...
/// @brief Asserts that a given vector/color/value is finite, and aborts if that
/// is not the case.
#define assert_finite(n, ctx)                                                  \
    if (!check_finite(n)) {                                                    \
        logger(EError,                                                         \
               "%s:%d : " #n " (= %s) is not finite",                          \
               relpath(__FILE__),                                              \
               __LINE__,                                                       \
               n);                                                             \
        ctx;                                                                   \
...
/// @brief Asserts that two vectors are orthogonal, and aborts if that is not
/// the case
#define assert_normalized(n, ctx)                                              \
    if (!check_normalized(n)) {                                                \
        logger(EError,                                                         \
               "%s:%d : " #n " (= %s) is not normalized",                      \
               relpath(__FILE__),                                              \
               __LINE__,                                                       \
               n);                                                             \
        ctx;                                                                   \
...
/// @brief Asserts that two given vectors are orthogonal, and aborts if that is
/// not the case.
#define assert_orthogonal(a, b, ctx)                                           \
    if (!check_orthogonal(a, b)) {                                             \
        logger(EError,                                                         \
               "%s:%d : " #a " (= %s) and " #b " (= %s) are not orthogonal",   \
               relpath(__FILE__),                                              \
               __LINE__,                                                       \
               a,                                                              \
               b);                                                             \
...
#else
#define assert_condition(condition, ctx) assert(condition)
#define assert_finite(n, ctx)
#define assert_normalized(n, ctx)
#define assert_orthogonal(a, b, ctx)
#endif
