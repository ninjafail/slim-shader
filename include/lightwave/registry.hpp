/**
 * @file registry.hpp
 * @brief Contains interfaces to register and query classes that can be
 * referenced in scene description files.
 */
...
/// @brief Keeps track of all classes that can be referenced in scene
/// description files.
class Registry {
public:
    ...
    template <typename C> struct Registrar {
        Registrar(const std::string &category, const std::string &name,
                  Constructor constructor) {
            Registry::constructors()[category][name] = constructor;
        }
    };

    /// @brief Checks whether a given category (e.g., "shape") of classes
    /// exists.
    static bool exists(const std::string &category);
    /// @brief Checks whether a given class (e.g., "sphere") within a given
    /// category of classes (e.g., "shape") exists.
    static bool exists(const std::string &category, const std::string &name);
    /// @brief Prints all available categories and classes to the provided
    /// output stream.
    static void listAvailable(std::ostream &os);
    /// @brief Creates an object of the given category and class by calling its
    /// constructor with the provided properties.
    static ref<Object> create(const std::string &category,
                              const std::string &name,
                              const Properties &properties);

private:
    using ConstructorMap =
        std::map<std::string, std::map<std::string, Constructor>>;
...
#define REGISTER_CLASS(Class, Category, Name)                                  \
    namespace lightwave {                                                      \
    static ref<Object> TOKENPASTE2(create_,                                    \
                                   __LINE__)(const Properties &properties) {   \
        try {                                                                  \
            /* do not use std::make_shared so error messages make more sense   \
             */                                                                \
            return ref<Class>(new Class(properties));                          \
        } catch (...) {                                                        \
            lightwave_throw_nested(                                            \
                "while creating " LW_STRINGIFY(Class) " object");              \
        }                                                                      \
    }                                                                          \
    static Registry::Registrar<Class>                                          \
        TOKENPASTE2(registrar_, __LINE__)(Category, Name,                      \
                                          TOKENPASTE2(create_, __LINE__));     \
    }
...
#define REGISTER_POSTPROCESS(Class, Name)                                      \
    REGISTER_CLASS(Class, "postprocess", Name)
...
#define REGISTER_INTEGRATOR(Class, Name)                                       \
    REGISTER_CLASS(Class, "integrator", Name)
